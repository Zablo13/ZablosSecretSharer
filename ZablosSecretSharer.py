### Zablos Secret Sharer V3.0.5 One Time Pad ###
import os
import sys
...
def translate_into_mastercode(message=None):
    ...
    if shares == 2:
        ...
        print(f'Share2/{shares}: {code2_str}')
    else:
...
def last_code(messageMaster, random_codes, UccMaster, shares):
    ...
    codes_global[f'Share{shares}'] = last_code_chars
    messageOut = ''.join(last_code_chars)
    print(f'Share{shares}/{shares}: {messageOut}')
